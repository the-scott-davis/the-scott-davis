...
4. **Stats have two cost tiers, and the cheap one is nearly everything.**
...
5. **Commit counts never come from walking repositories.** They come from
...
6. **`make build` is the offline path.** It renders from `cache/loc.json` and
...
7. **Layout is computed, not measured.** `render.py` reserves
...
10. **Pick the mode from the source art's LUMINANCE contrast, not its subject.**
   ...
   of luminance difference (out of 255) between face and shirt — 99 in colour —
...
11. **An ASCII portrait is set at its own font size** (`card.portrait_font_size`
...
12. **The RLE in `pixel_art.load_boxes` must stay lossless.** It exists purely to
...
13. **Unknown placeholders are an error, not a blank.** `{typo}` raises a
